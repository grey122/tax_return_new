...
class MockFaceBookLoginAceessToken extends Mock implements FacebookAccessToken {
}
...
void main() {
  ...
  group('AuthenticationRepository', () {
    ...
    group('loginWithFaceBook', () {
      const fbToken = 'fb-token';
      final facebookLoginResult = MockFaceBookLoginResult();
      final facebookAccessToken = MockFaceBookLoginAceessToken();

      setUp(() {
        when(facebookAccessToken.token).thenReturn(fbToken);
        when(facebookLoginResult.accessToken)
            .thenAnswer((_) => facebookAccessToken);
        when(
          facebookLogin.logIn(['email']),
        ).thenAnswer((_) async => facebookLoginResult);
      });

      test('calls sigin in authentictaion and signincreditial', () async {
        when(facebookLoginResult.status)
            .thenReturn(FacebookLoginStatus.loggedIn);
        await authenticationRepository.logInWithFaceBook();
        verify(facebookLogin.logIn(['email'])).called(1);

        verify(firebaseAuth.signInWithCredential(any)).called(1);
      });
      test('succeseds when facebook signin succeceds', () async {
        expect(authenticationRepository.logInWithFaceBook(), completes);
      });
    });
    ...
    group('logInWithEmailAndPassword', () {
      ...
      test(
        ...
        expect(
          ...
          throwsA(isA<FirebaseFailureMessage>()),
        );
      });
    });
    ...
    group('logOut', () {
      test('calls signOut', () async {
        ...
        when(facebookLogin.logOut()).thenAnswer((_) async => null);
